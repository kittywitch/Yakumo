...
app.config['UPLOAD_FOLDER'] = f"{base_path}/files"
#db = SQLAlchemy(app)
...
def uploads():
    if request.method == 'POST':
        content = request.json
        upload_list = []
        for filename in os.listdir(os.path.join(app.config['UPLOAD_FOLDER'], f"./{content['username']}")):
            full_name = os.path.join(
                app.config['UPLOAD_FOLDER'], f"./{content['username']}/{filename}")
            filetime = int(creation_date(full_name))
            url = url_for('uploaded_file',
                          username=content['username'], filename=filename)
...
def upload():
    ...
    if request.method == 'POST' and "username" in request.form:
        ...
        user_path = os.path.join(
            app.config['UPLOAD_FOLDER'], f"./{request.form['username']}")
        if not os.path.exists(user_path):
            os.makedirs(user_path)
        file.save(os.path.join(user_path, filename))
        app.logger.info(
            f"User {request.form['username']} is uploading \"{os.path.join(user_path, filename)}\" from \"{request.environ.get('HTTP_X_REAL_IP', request.remote_addr)}\".")
        return json.dumps({"url": url_for('uploaded_file', username=request.form['username'], filename=filename), "status": "successful"})
    else:
        return '', 401
